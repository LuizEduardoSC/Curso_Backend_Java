...
public class ClienteController implements Serializable {
	...
	public void add() {
		try {
            ...
            e.printStackTrace(); // <-- ISSO VAI SALVAR SUA VIDA NOS TESTES
	...
	private void removerCaracteresInvalidos() {
        // Pega a String da tela, remove tudo que não é número e converte para Long
        if (this.cpfMask != null && !this.cpfMask.trim().isEmpty()) {
            Long cpf = Long.valueOf(this.cpfMask.replaceAll("\\D", ""));
            this.cliente.setCpf(cpf);
        }
        
        if (this.telMask != null && !this.telMask.trim().isEmpty()) {
            Long tel = Long.valueOf(this.telMask.replaceAll("\\D", ""));
